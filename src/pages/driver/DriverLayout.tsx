...
import { ArrowLeft, Bike, LayoutDashboard, LogOut, Package } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

const nav = [
  { to: '/livreur', end: true, label: 'Tableau de bord', icon: LayoutDashboard },
  { to: '/livreur/courses', end: false, label: 'Mes courses', icon: Package },
];
    ...
    <div className="min-h-screen bg-gray-50 flex">
      <aside className="hidden md:flex w-64 flex-col bg-[#1F2937] text-white">
        <div className="p-5 border-b border-gray-700">
          ...
          <p className="mt-3 text-xs text-gray-400">Espace livreur</p>
          <p className="font-bold truncate">{user?.fullName}</p>
          {user?.driver?.driverCode && (
            <p className="text-xs font-mono text-[#FF6B00] mt-1">{user.driver.driverCode}</p>
          )}
        </div>
        ...
        <div className="p-3 border-t border-gray-700 space-y-1">
          <Link to="/" className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm text-gray-300 hover:bg-gray-800">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden bg-white border-b px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm truncate">{user?.driver?.driverCode || 'Livreur'}</span>
          </div>
          <Link to="/" className="text-xs text-[#FF6B00] font-semibold">Site</Link>
        </header>
